...
void InstructionEncoderLLUMC::visitTruncInst(llvm::TruncInst &I) {
    ...
    //TODO visist LShr to finish example, visitLShr is not in InstVisitor
    const bool alreadyVisited = m_alreadyVisited.find(I.getName()) != m_alreadyVisited.end();
    if(alreadyVisited){
        llvm::outs()<< "ALREADY VISITED CONTINUE\n";
    }else {
        ...
        llvm::outs() << *I.getSrcTy() << ", " << *I.getDestTy()<< "\n";
        //tmp63Dash = tmp62;
        llvm::outs() << I.getName() << "\n";
        llvm::outs() << I.getOperand(0)->getName()<< ", " <<"\n";
        ...
        //SMT::SatCore *satCore = m_smtContext->getSatCore();
        SMT::BVExp *bvName = m_SMTTranslator.createBV(I.getName().str() + "Dash", I.getDestTy()->getIntegerBitWidth());
        //todo trunc it
        SMT::BVExp *bvTrunc = bv1;


        const bool is_in = m_variableSet.find(I.getName()) != m_variableSet.end();
        if (is_in) {
            std::string name = I.getName();
            SMT::BVExp *bvExp = m_SMTTranslator.createBV(name + "Dash", I.getDestTy()->getIntegerBitWidth());
